...
subtitle: "A practical, evidence-led strategy that fits on one NVIDIA DGX Spark"
version: "1.1"
date: "21 August 2026"
...
**Revision control.** Version 1.1 corrects execution topology, controller gates, machine contracts and operational commands. The research cut remains 21 August 2026; this revision does not imply that later literature was reviewed.
...
## 1.3 Traceability to the wider P42-KB project

This guide is a supporting work package, not a new set of project requirements. This is a navigation table, not a compliance claim: the guide contributes evidence, but only the controlled project acceptance record can show that a requirement or deliverable is satisfied. The table below shows which governing objectives it helps to prove and where the evidence is produced.

| Governing item | What it requires in ordinary language | Where this guide supplies evidence | PoC deliverable |
|---|---|---|---|
| `KB-HLR-033` | evaluate the capability on a representative engineering benchmark | Chapters 7–12 | D3 |
| `KB-HLR-034` | separate retrieval, evidence coverage, answer quality, grounding and multi-source reasoning | §7.2 and Chapter 12 | D3 |
| `KB-HLR-035`, `OBJ-09` | recognise insufficient, ambiguous and contradictory evidence | §3.9, §9.6 and §12.3–12.4 | D3, D10 |
| `KB-HLR-036` | include engineering investigations, not only simple questions | §4.3, §11.12 and protected-real validation | D3, D9 |
| `KB-HLR-059`, `KB-HLR-062` | keep an auditable and reproducible record | §6.8, Chapters 9–10 and Appendices A–B | D3, D10 |
| `KB-HLR-060`, `KB-HLR-061` | version corpus changes and keep AI-derived material separate from authority | §6.2, §8.3, §11.13 and B.1–B.2 | D3, D10 |
| `KB-HLR-065` | declare and evaluate language coverage | §9.3, §12.5 and the activation record | D3, D10 |

The Project Definition remains authoritative for the requirement wording, and the PoC Plan remains authoritative for deliverable scope and timing.
...
For routing, use a **protection tier** as well as the descriptive class. `PUBLIC_CLEARED` is tier 0, `PUBLIC_RESTRICTED` is tier 1, and both `AIRBUS_CONTROLLED` and `AIRBUS_DERIVED` are tier 2. `UNKNOWN` is not a low tier; it is unordered and stays unopened until an authorised decision assigns a class. A new output may never have a lower protection tier than any input. Preserve lineage as follows:

| Highest-risk input | Minimum output treatment |
|---|---|
| only `PUBLIC_CLEARED` | `PUBLIC_CLEARED`, subject to output rights review |
| any `PUBLIC_RESTRICTED`, no Airbus input | `PUBLIC_RESTRICTED` |
| any `AIRBUS_CONTROLLED` | byte-preserving copies remain `AIRBUS_CONTROLLED`; extracts, prompts, embeddings, analyses and generated material are `AIRBUS_DERIVED` |
| any `AIRBUS_DERIVED` | `AIRBUS_DERIVED` |
| any `UNKNOWN` | no processing output; route the unopened item to security quarantine |
...
```{.yaml data-p42-contract="job-envelope/1.1"}
...
escalation_rule_ids: [CRITICAL_CONTENT_UNREADABLE, EVIDENCE_CONFLICT]
forbidden_actions:
  - network
  - write_source_record
...
The model terminal status enum is `complete`, `needs_escalation`, `data_boundary_blocked` or `tool_failure`. This is separate from orchestrator workflow state. Semantic non-answerability is a valid `complete` payload, not a wrapper failure. Model confidence is triage metadata, never an acceptance score. One transient infrastructure retry and one field-bounded repair are allowed; a second content failure is quarantined. A model/configuration change creates a new `config_id` and never silently replaces a failed run.
...
Use two deliberately different implementation gates:

- **Protected execution baseline (required before any protected model call):** the classification registry, no-egress/access enforcement, signed input/output hashes, frozen configuration, task/response schema validation and immutable run manifest. The committed protected-real retrieval/answer benchmark may run through a small reproducible benchmark runner once this baseline passes; it does not have to wait for the full production state machine.
- **AI-first batch controller (required before reduced-human or automated-corpus headline claims):** durable job state, crash/resume, retry/field repair, model-review routing, quarantine, probability sampling, watchdogs and the consolidated human queue. Large archetype observation or synthetic production batches may not bypass this gate.

The full AI-first batch route is **not operational** until a tested controller can:
...
Interactive copy/paste is Phase A exploration only. It is forbidden for Phase B and cannot produce headline evidence. A small local benchmark script is not interactive copy/paste: it may run protected-real cases after the protected execution baseline passes, provided it freezes evidence packs, validates typed responses and records the complete B.1 manifest. Prompt rules are defence in depth; the operating-system/container policy is the actual boundary.
...
Set the statistical expectation at activation. If the illustrative 60-bundle study audits only the 20% clean-pass floor and observes zero misses in 12 independent bundles, the one-sided 95% upper bound is still about 22% (the rough rule-of-three value is 25%). Reaching a roughly 5% zero-miss bound needs about 59 independent audits—almost full review of a 60-bundle study. Therefore a low residual-risk claim may be possible only with a much larger independent evidence budget; otherwise report descriptive feasibility and the observed numerator/denominator. The 20% value is an operational floor, not a quality guarantee or cap.
...
::: {.plain}
**In simple words:** Phase A may use the internet because it handles only `PUBLIC_CLEARED` material. Use cloud AI to perform the research, coding, public fixture creation, criticism and reporting that would otherwise consume project time. Acquire and smoke-test the complete route before the airlock. After the trusted rebuild, replay a signed public vertical-slice acceptance test before Airbus import; longer public bake-offs may then continue offline in separate frozen runs.

On the single-Spark plan, connected Phase A on the Spark is deliberately short: acquire, smoke-test and over-provision the required assets in weeks 1–2, then freeze the signed transition bundle and rebuild the Spark as trusted Phase B. Public bake-offs may continue **offline** after that transition because public data do not become unsafe merely by being processed locally. Cloud research can continue on a separate approved connected workstation, but any new artefact needed by Phase B must pass a controlled incremental import and affected-component requalification; it is never copied directly into the protected environment.

The A0–A8 labels are logical qualification steps, not a claim that every computation stays on the connected image. Finish every network-dependent acquisition and smoke test before the airlock. Record the actual `execution_zone` in every run, including public rehearsal runs performed after the Spark enters `TRUSTED_PHASE_B`.
:::
...
date --iso-8601=seconds > "$P42_ROOT/logs/start_time.txt"
uname -a > "$P42_ROOT/logs/uname.txt"
cat /etc/os-release > "$P42_ROOT/logs/os-release.txt"
free -h > "$P42_ROOT/logs/memory.txt"
df -h > "$P42_ROOT/logs/storage.txt"
docker version > "$P42_ROOT/logs/docker-version.txt"
nvcc --version > "$P42_ROOT/logs/cuda-version.txt"

sed -n '1,40p' "$P42_ROOT/logs/start_time.txt" "$P42_ROOT/logs/uname.txt" \
  "$P42_ROOT/logs/memory.txt" "$P42_ROOT/logs/storage.txt"
```
...
### Build the minimum protected runner, then the batch controller

Use cloud AI on the public schemas to implement both layers defined in §6.8. First build the protected execution baseline and a small reproducible benchmark runner: classification and lineage registry, policy-selected execution profile, no-egress/access enforcement, signed hashes, frozen evidence packs, typed request/response validation and a B.1 run manifest. This layer is the gate for protected-real headline evidence.

Then add the AI-first batch controller needed for archetype observation, generation and any reduced-human workload claim: append-only SQLite state, atomic attempt hashes, retry/field-repair/quarantine routing, deterministic probability sampling, watchdogs, crash/resume and one human-queue export. Test it with deliberately wrong classes, symlinks, prompt-injection documents, unknown fields, missing evidence, timeouts, swap/disk alarms, duplicate jobs and crash/restart. A separate model/provider reviews the public source only when separately approved; security-sensitive changes receive human code review.

**Done when.** Before the airlock, the runner passes public boundary, schema, failure and reproducibility fixtures and is frozen into the transition bundle. After the trusted rebuild, it must pass the installed no-egress, access-profile and offline replay acceptance tests before any protected data is introduced. The full controller additionally reproduces a complete public micro-batch after restart and rejects manual class promotion. Until the relevant layer passes, its corresponding output is exploratory and cannot support that headline claim.
...
Create gold checks for the elements that affect P42-KB. `DRAFT_PUBLIC_PARSER_GOLD_V2` may let cloud AI propose blocks/tables and explanations, but benchmark/deterministic truth is preferred; a calibrated human audit confirms only the diverse scored subset:
...
A successful evidence-bounded conclusion that the question is `not_answerable`, `ambiguous` or affected by `conflicting_authority` still uses worker wrapper `status=complete`: the semantic outcome belongs in this payload. Use `needs_escalation` when the worker cannot produce a valid payload and a closed routing rule can send the unresolved case to another worker or a person.
...
::: {.check}
**Phase A exit:** the protected execution baseline passes its boundary, schema and reproducibility fixtures; all transition assets are rights-cleared, pinned, scanned, signed and rebuildable offline. If Gate 3 may authorise archetype/synthetic batch work, the full batch controller must also reproduce a public micro-batch after crash/restart and its measured exception load must fit the budget. Otherwise the full batch controller may be deferred without blocking the committed protected-real benchmark.
:::
...
Run the single fail-closed procedure in Appendix A.8 only after every wheel, dataset, container archive, model and rehearsal asset is present. Do not maintain a second checksum implementation here. A.8 sets `LC_ALL=C`, checks discovery, sorting and hashing separately, rejects an empty tree plus symlinks/hard links/special files, and writes manifests through temporary files.

The staging tree must be quiesced and write-inhibited during enumeration and hashing; otherwise a file can change between the two operations. Sign the checksum and file-set record using the approved organisational mechanism. On import, the trusted verifier rejects missing and unexpected files before recomputing the expected checksums from the signed, already-validated NUL file list. A checksum proves byte identity, not trustworthiness or licence clearance.

## 10.3 Preserve actual container content

An image name is not an offline asset. Use the canonical Appendix A.3 procedure to save the image layers, validate the compressed archive and record a non-empty immutable `RepoDigest`. Replace `<tested-spark-tag>` with the qualified tag from NVIDIA's current Spark recipe. Do not copy a floating `latest` tag into the freeze record.
...
| End of week 2 | move the one Spark through the signed airlock and trusted rebuild | no protected work begins until the transition exit passes |
...
| Gate 3 | choose and authorise at most one bounded optional extension: B0 for one pilot family **or** one specialist FA vertical; defer both if core hardening lacks capacity | if B0 is selected, its result alone can authorise wider B1–B12 archetype work |
...
## A.0 Preflight the host and approved workspace
...
## A.11 Run the required model-qualification interface
...
Install complete schema files in the controller and pass the selected schema to structured decoding. The following is the minimum combined v1.1 contract; production versions may add stricter conditional rules but must not remove `additionalProperties: false`.
...
| Benchmark | What it is useful for | Code/evaluator position | Dataset/document position | v1.1 disposition |
|---|---|---|---|---|
...
## G.1 Material changes in v1.1

- Resolved the one-Spark topology: connected acquisition/smoke work ends in week 2, the signed airlock follows, and longer public bake-offs run offline on frozen assets.
- Split control into a protected execution baseline required for any protected model call and a full AI-first batch controller required for scaled archetype/synthetic work and reduced-human claims.
- Replaced the circular Gate 3 instruction with an optional bounded B0 pilot; at most one of that family pilot and the specialist FA vertical enters the weeks 7–9 extension window.
- Made the flagship job and answer examples conform to versioned `/1.1` contracts; added a closed action registry, referential-integrity checks, one evidence-ID grammar and an explicit `ambiguous` answerability state.
- Removed wrapper `abstain`: valid evidence-bounded non-answers are `complete` task payloads, while inability to form a payload routes through a closed escalation/failure status.
- Defined data-class propagation as a signed transition matrix rather than an undefined total ordering.
- Replaced fail-open checksum, container-export and API-smoke patterns with checked stages, exact assertions, immutable-digest checks and negative-test expectations.
- Corrected the Qdrant network example so internal networking and host port publishing are mutually exclusive profiles.
- Added project-requirement/deliverable navigation, explicit 70B-arm disposition, unresolved-acronym handling and clearer non-normative revision metadata.
