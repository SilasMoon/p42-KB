...
> **Presentation and strategy disposition in guide v1.2.** Version 1.2 retains those controls but reorganises the document for progressive disclosure. The mandatory narrative now follows purpose → strategy → programme gates → operating design → activation → public qualification → airlock → protected-real Find/Answer/Connect → final confirmation and decision. Commands, contracts, dated model choices, research evidence, sampling mathematics and the optional B1–B12 family study have one canonical home in linked appendices. A new protected-real Phase B runbook, core-only decision matrix, parser-gold procedure, classification transition matrix, product-envelope template and pre-registration checklist close the practical gaps found during the reader-centred restructure. This note does not turn the present file into a new review of v1.2; the original v1.0 findings and evidence remain historical.
